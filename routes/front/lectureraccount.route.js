...
router.post( '/login', async function ( req, res, next )
{
	passport.authenticate( 'lecturer', function ( err, user, info )
	{
		...
		if ( !user ) { return res.render( 'vwLecturers/login', { layout: false, err_message: 'Invalid username or password2.' } ); }
...
router.get( '/profile', auth, async function ( req, res )
{
	...
	if ( userInfo !== null )
		userInfo.l_DOB = moment( userInfo.l_DOB ).format( 'MM-DD-YYYY' );
